...
#1 Добавление питомца с некорректными данными
def test_add_new_pet_simple_with_invalid_data(name="House", animal_type='Doctor', age='toomuch'):
    ...
    assert status == 400
    assert result['name'] == name
#2 Проверка на ввод некорректного логина и пароля
def test_get_api_key_for_invalid_user(email=invalid_email, password=invalid_password):
    ...
    #В случае ввода корреткных данных возвращаем токен авторизации
        ...
        assert status == 200
        assert 'key' in result
#3 Согласно документации API, допустимы только форматы JPG, JPEG, PNG форматы. Подставляем недокументированный формат файла, тест возвращает 200ый статус, ожидаемый код 400(Баг!)
...
#4 В случае, если производится попытка отправить объект не в графическом формате, возвращается ошибка 500(Сервер не может обработать вложение), вместо ошибки 400, что является багом.
